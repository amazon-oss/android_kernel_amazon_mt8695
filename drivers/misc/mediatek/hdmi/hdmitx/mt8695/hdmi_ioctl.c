...
int hdmi_video_config(HDMI_VIDEO_RESOLUTION vformat)
{
	...
	if (send_fake_hpd) {
		...
		send_fake_hpd = false;
		TX_DEF_LOG("hdmi_video_config: fake hpd sent\n");
	}
				      ...
				      size_t count)
{
	...
	if (strncmp(buf, "dbglevel:", 9) == 0) {
		...
		if (ret == 1) {
			...
			sprintf(hdmi_debug_normal_buffer + strlen(hdmi_debug_normal_buffer),
				"log new=0x%x\n", hdmidrv_log_on);
		} else {
			snprintf(hdmi_debug_normal_buffer, sizeof(hdmi_debug_normal_buffer), "error,buf=%s\n", buf);
				 ...
				 struct device_attribute *devattr, char *buf)
{
	...
	sprintf(hdmi_hdr_status_buffer + strlen(hdmi_hdr_status_buffer), "\nret=%d\n", ret);
