...
atomic_t hw_count[2] = {ATOMIC_INIT(0), ATOMIC_INIT(0)};


void mdp_imgresz_enable_clk_id(int hw_id)
{
/*
**	param hw_id: 0 / 1 to enable imgresz HW0 / HW1
*/
	if (hw_id >= ARRAY_SIZE(hw_count)) {
		MDP_ERR("invalid hw id:%d\n", hw_id);
		return;
	}

	if (atomic_inc_return(&hw_count[hw_id]) == 1) {
		int ret = 0;

		MDP_LOG("enable HW %d clk\n", hw_id);
		ret = imgresz_clk_on(hw_id);
		if (ret != 0) {
			MDP_ERR("enable HW %d clk fail", hw_id);
			return;
		}
	}
...
void mdp_imgresz_disable_clk_id(int hw_id)
{
/*
**	param hw_id: 0 / 1 to disable imgresz HW0 / HW1
*/
	int value = 0;
	...
	value = atomic_dec_return(&hw_count[hw_id]);
	if (value == 0) {
...
{
	/*
	** enable/disable imgresz clock according to the HW Task used.
	*/
	IMGRESZ_TICKET ticket = pTask->task.imgresz_task.ticket;
	enum imgresz_ticket_fun_type type = mdp_imgresz_get_function_type(pTask->mdp_command.src_buffer.color_format,
		pTask->mdp_command.src_buffer.width,
		pTask->mdp_command.src_buffer.height);

	if (type == IMGRESZ_FUN_ONEPHASE_2HW ||
		type == IMGRESZ_FUN_UFO_2HW) {
		if (enable_clock) {
			mdp_imgresz_enable_clk_id(0);
			mdp_imgresz_enable_clk_id(1);
		} else {
			mdp_imgresz_disable_clk_id(0);
			mdp_imgresz_disable_clk_id(1);
		}
		return;
	}


	if (ticket == TICKET_FROM_HWID(0)) {
		/* use HW 0 */
		if (enable_clock)
			mdp_imgresz_enable_clk_id(0);
		else
			mdp_imgresz_disable_clk_id(0);
	} else if (ticket == TICKET_FROM_HWID(1)) {
		/* use HW 1 */
		if (enable_clock)
			mdp_imgresz_enable_clk_id(1);
		else
...
{
	/* ufo format */
	if (DP_COLOR_GET_UFP_ENABLE(format)) {
#if 0
...
int32_t mdp_imgresz_dispatch_hw(void *ignore)
{
	...
	do {
		...
		/* if no task or hw not ready, sleep */
		if ((pTask == NULL) || !mdp_imgresz_get_ticket(pTask, &pTask->task.imgresz_task.ticket)) {
			wait_woken(&wait, TASK_INTERRUPTIBLE, MAX_SCHEDULE_TIMEOUT);
			continue;
		}

		/* remove task from trigger list. */
		spin_lock_irqsave(&global_mdp_task.trigger_imgresz_task_list.spin_lock, flags);
		list_del_init(&pTask->list_entry);
		spin_unlock_irqrestore(&global_mdp_task.trigger_imgresz_task_list.spin_lock, flags);

		/* enable task clock*/
		mdp_imgresz_mgr_task_clk(pTask, true);
...
{
	struct mdp_task_struct *pTask = (struct mdp_task_struct *)privdata;
