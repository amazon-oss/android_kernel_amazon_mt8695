...
static BOOL fg_enable_faker_hdr;
		...
		i = plane_id;
		{
			...
			if (!list_empty(&(OSD_Configed_Head[i]))) {
				...
				if (pBuffList->vsync_cnt < vsync_cnt) {
					...
					OSDDBG("free pre-fence idx[%ld]:%d", vsync_cnt,
						pBuffList->buffer_info.config.pre_index);
				} else {
...
int osd_config_sw_register(struct disp_osd_layer_config *osd_overlay_buffer_info)
{
	INT32 i4Ret = 0;
	UINT32 u4Region=0;
	uint32_t osd_hstart=0, osd_vodd=0, osd_veven=0;
	...
	OSD_PRINTF(OSD_CONFIG_SW_LOG,
		"osd_config_sw_register u4Plane=%d ,layer_enable =%d, src_fmt=%x, alpha:%d, alpha_en:%d\n",
	...
	if (u4SBS == 1) {
		...
		if (i4Ret < 0)
			OSD_PRINTF(OSD_CONFIG_SW_LOG, "get free region fail\n");
	...
	if (osd_swap) {
		...
		/*get active info*/
		i4Ret = disp_path_get_active_zone(mix_layer_id, osd.res_chg[u4Plane].cur_res_mode,
...
static int disp_mgr_post_osd_buffer(disp_osd_input_config *buffer_info)
{
	...
	if (osd_drop_command_check(i, buffer_info)) {
		OSD_LOG_I("Invalid layer res, drop this layer and free ion_handle=%p\n",
...
int disp_osd_start(struct disp_hw_common_info *info, unsigned int layer_id)
{
	...
	if (osd.osd_stop_ctl.osd_stop_status[osd_layer_id] == OSD_LAYER_STOPPED) {
		...
		if (osd_layer_id == OSD_PLANE_2
			&& osd.osd_stop_ctl.osd_stop_status[OSD_PLANE_1]
			== OSD_LAYER_STOPPED) {
			osd_clk_enable(osd.res_chg[OSD_PLANE_1].cur_res_mode,
				true, OSD_PLANE_1);
			fg_osd_always_update_frame[OSD_PLANE_1] = 3;
			if	(fg_update_after_init)
				fg_update_after_init = false;
			osd.osd_stop_ctl.osd_stop_status[OSD_PLANE_1]
				= OSD_LAYER_START;
			osd.osd_stop_ctl.osd_stop_pts[OSD_PLANE_1] = 0;
			OSD_LOG_I("enable osd plane_%d clk \n", OSD_PLANE_1);
		} else if (osd_layer_id == OSD_PLANE_1
			&& osd.osd_stop_ctl.osd_stop_status[OSD_PLANE_2]
			== OSD_LAYER_STOPPED) {
			osd_clk_enable(osd.res_chg[OSD_PLANE_2].cur_res_mode,
				true, OSD_PLANE_2);
			fg_osd_always_update_frame[OSD_PLANE_2] = 3;
			osd.osd_stop_ctl.osd_stop_status[OSD_PLANE_2]
				= OSD_LAYER_START;
			osd.osd_stop_ctl.osd_stop_pts[OSD_PLANE_2] = 0;
			OSD_LOG_I("enable osd plane_%d clk \n", OSD_PLANE_2);
		}
	}
...
int disp_osd_dump(uint32_t level)
{
	...
	UINT32 u4PlaneChecksum, u4SclChecksum;
	UINT32 u4PreMultiplied;
	BOOL status;
	UINT32 u4Region;
	OSD_BASE_UNION_T	*prHwOsdFmtReg_dump;
	OSD_PLA_CORE_UNION_T	*prHwOsdPlaneReg_dump;
	OSD_SC_UNION_T		*prHwOsdSclReg_dump;
	OSD_PREMIX_UNION_T	*prHwOsdPremixReg_dump;
	...
	char *SRC_DATA[2]			=	{"Nor Premulti", "Premulti"};
	...
	prHwOsdPremixReg_dump			=	(OSD_PREMIX_UNION_T *)osd.osd_reg.osd_pmx_reg_base;

	OSD_LOG_I("<<<<<<<<<<< OSD INFO BEGIN >>>>>>>>>>\n");
	for (u4OsdNum = 0; u4OsdNum < OSD_PLANE_MAX_NUM; u4OsdNum++) {
		OSD_LOG_I("**********OSD-%s layer **********\n", OSD_LAYER[u4OsdNum]);
		prHwOsdFmtReg_dump		=	(OSD_BASE_UNION_T *)osd.osd_reg.osd_fmt_reg_base[u4OsdNum];
		prHwOsdPlaneReg_dump	=	(OSD_PLA_CORE_UNION_T *)osd.osd_reg.osd_pln_reg_base[u4OsdNum];
		prHwOsdSclReg_dump		=	(OSD_SC_UNION_T *)osd.osd_reg.osd_scl_reg_base[u4OsdNum];
		...
		if (osd_color_fmt == OSD_CM_RGB565_DIRECT16)
			...
			src_color_format = SRC_COLOR_FORMAT[src_Asel][src_Yrsel];
		...
		dst_x		=	u4OsdNum ?
					prHwOsdFmtReg_dump->rField.u4Osd3HStart :
					prHwOsdFmtReg_dump->rField.u4Osd2HStart;
		dst_y		=	u4OsdNum ?
					prHwOsdFmtReg_dump->rField.u4Osd3VStart :
					prHwOsdFmtReg_dump->rField.u4Osd2VStart;
		dst_width	=	prHwOsdSclReg_dump->rField.u4DstHSize;
		dst_height	=	prHwOsdSclReg_dump->rField.u4DstVSize;
		if (u4OsdNum == 0) {
			u4PlaneChecksum = prHwOsdFmtReg_dump->rField.u4Osd2CheckSum;
			u4SclChecksum   = prHwOsdFmtReg_dump->rField.u4Osd2ScCheckSum;
			u4PreMultiplied = prHwOsdPremixReg_dump->rField.mix2_pre_mul_a;
		} else {
			u4PlaneChecksum = prHwOsdFmtReg_dump->rField.u4Osd3CheckSum;
			u4SclChecksum   = prHwOsdFmtReg_dump->rField.u4Osd3ScCheckSum;
			u4PreMultiplied = prHwOsdPremixReg_dump->rField.mix1_pre_mul_a;
		}
		...
		if (osd.plug_out[OSD_PLANE_1].fg_hdmi_plug_out) {
			...
			current_hdmi_resolution	=	osd.res_chg[u4OsdNum].cur_res_mode;
			OSD_LOG_I("** now hdmi status is plug_out, current HDMI resolution is %s\n",
				HDMI_RESOLUTION_MODE[current_hdmi_resolution]);
		} else {
			current_hdmi_resolution	=	osd.res_chg[u4OsdNum].osd_res_mode;
			OSD_LOG_I("** HDMI resolution is %s\n", HDMI_RESOLUTION_MODE[current_hdmi_resolution]);
			if (status)
				current_fence_index	=
							osd.osd_current_frame[u4OsdNum]->buffer_info.config.index;
		}
		OSD_LOG_I("** Layer status: %s\n", LAYER_STATUS[status]);
		if (status) {
			OSD_LOG_I("** src_data: %s\n", SRC_DATA[u4PreMultiplied]);
			OSD_LOG_I("** src_rec :(%4d,%4d,%4d,%4d) src_format:%s\n",
				src_x, src_y, src_width, src_height, src_color_format);
			OSD_LOG_I("** dst_rec :(%4d,%4d,%4d,%4d) dst_color_mode:%s\n",
				dst_x, dst_y, dst_width, dst_height, DST_COLOR_MODE[dst_color_mode]);
			if (level >= 1) {
				OSD_LOG_I("** now osd playing fence index=%d\n",  current_fence_index);
				OSD_LOG_I("** now still have %d buffer in configed queue not play\n",
					pre_fence_cnt-current_fence_index);
				OSD_LOG_I("** fence_cnt=%d,pre_fence_cnt=%d,timeline_cnt=%d pre_timeline_cnt=%d\n",
					fence_cnt, pre_fence_cnt, timeline_cnt, pre_timeline_cnt);
				OSD_LOG_I("** config_cnt=%ld,release_cnt=%ld\n",
					config_cnt[u4OsdNum], release_cnt[u4OsdNum]);
			}
			if (level >= 2)
				OSD_LOG_I("** PlaneChecksum:0x%x, SclChecksum:0x%x\n", u4PlaneChecksum, u4SclChecksum);
		}
		OSD_LOG_I("******************************\n");
	}
