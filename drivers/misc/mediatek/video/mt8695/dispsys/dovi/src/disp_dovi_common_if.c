...
UINT32 dovi_update_graphic_info(void)
{
	...
	if (gmax != 0)
		...
		p_cp_param->graphic_max_lum = 3000000;	/* align 2.4 C model -> refer to TB39 */
		...
		if (hdr_metadata->dr_range == DISP_DR_TYPE_DOVI) {
			...
			if (dovi_md_info->svp) {
				...
				*p_rpu_bs_len = dovi_md_info->len;
				...
				dovi_rpu("svp process frame:%d, rpu_len:%d, sec_handle:%d\n",
					*p_frame_num, dovi_md_info->len, dovi_md_info->sec_handle);
			} else {
				*p_svp = false;
				/* remove rpu NAL type */
