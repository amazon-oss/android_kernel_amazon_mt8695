...
static ssize_t mt_gpio_store_pin(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "mode", 4) && (sscanf(buf+4, "%d %d", &pin, &val) == 2)) {
		...
		dbg_start = val;
	} else if (!strncmp(buf, "rr", 2) && (sscanf(buf+2, "%x", &pin) == 1)) {
		regmap_read(pctl->regmap1, pin, &val);
		pr_emerg("reg : 0x%x : 0x%x\n", pin, val);
	} else if (!strncmp(buf, "wr", 2) && (sscanf(buf+2, "%x %x", &pin, &val) == 2)) {
		regmap_write(pctl->regmap1, pin, val);
		pr_emerg("reg : 0x%x : 0x%x\n", pin, val);
	} else {
		pr_emerg("command not support\n");
	}
